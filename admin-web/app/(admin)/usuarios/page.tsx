...
import CrearUsuarioModal from "@/app/_components/usuarios/CrearUsuarioModal";
...
export default function UsuariosPage() {
  ...
  return (
    <div className="p-8">
          ...
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50/50">
              {["Nombre ↕", "Correo electrónico ↕", "Teléfono ↕", "Plan actual ↕", "Objetivo ↕", "Último acceso ↕", "Estado ↕", "Acciones"].map((h) => (
                <th key={h} className="text-left px-4 py-3.5 text-xs font-semibold text-gray-500">
                  {h}
                </th>
              ))}
            </tr>
      ...
      {showCreate && <CrearUsuarioModal onClose={() => setShowCreate(false)} />}
    </div>
  );
}
