...
export async function fetchMedidasHistorial(userId: string, limit = 6): Promise<Medida[]> {
  const { data, error } = await supabase
    .from("medidas")
    .select("*")
    .eq("user_id", userId)
...
export async function createMedida(userId: string, input: MedidaInput): Promise<Medida> {
  const { data, error } = await supabase
    .from("medidas")
